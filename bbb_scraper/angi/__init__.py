...
its own package rather than force-fit into bbb_scraper/scraping|parsing:
  ...
  - client.py -- the network part: no proxy needed (confirmed no bot-wall
    all session, unlike bbb.org), but still paced -- repeated requests to
    *one* host (angi.com itself), unlike webcheck's one-request-per-
    different-host pattern, so it gets BBB-style politeness pacing rather
    than webcheck's higher, many-different-hosts concurrency.
